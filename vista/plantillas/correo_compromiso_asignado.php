 ...
 * @var string      $comiteTitulo
 * @var string|null $empresaNombre
 * @var string      $descripcion
 * @var string|null $fechaLimiteFormateada
 * @var string      $urlLogin
 */
?>
<!doctype html>
        ...
        <table role="presentation" width="600" cellpadding="0" cellspacing="0" bgcolor="#ffffff" style="background-color:#ffffff;border-radius:8px;overflow:hidden;max-width:600px;width:100%;">
          ...
          <tr>
            <td bgcolor="#ffffff" style="background-color:#ffffff;padding:32px;color:#22404b;">
              <h2 style="margin:0 0 16px;color:#22404b;">Nuevo compromiso asignado</h2>
              <p style="margin:0 0 20px;color:#333;line-height:1.5;">
                Hola<?= !empty($nombreDestinatario) ? ' ' . htmlspecialchars($nombreDestinatario) : '' ?>, se te asignó un nuevo compromiso en el comité
                <strong><?= htmlspecialchars($comiteTitulo) ?></strong>
                del <strong>Centro de Control Gerencial — Zona Franca Internacional de Pereira</strong>.
              </p>

              <table role="presentation" width="100%" cellpadding="8" cellspacing="0" style="border-collapse:collapse;margin-bottom:20px;">
                <tr>
                  <td bgcolor="#f8fafb" style="border:1px solid #e2e6e8;background-color:#f8fafb;width:40%;color:#22404b;"><strong>Comité</strong></td>
                  <td bgcolor="#ffffff" style="border:1px solid #e2e6e8;background-color:#ffffff;color:#333;"><?= htmlspecialchars($comiteTitulo) ?></td>
                </tr>
                ...
                <?php endif; ?>
                <?php if (!empty($fechaLimiteFormateada)): ?>
                <tr>
                  <td bgcolor="#f8fafb" style="border:1px solid #e2e6e8;background-color:#f8fafb;color:#22404b;"><strong>Fecha límite</strong></td>
                  <td bgcolor="#ffffff" style="border:1px solid #e2e6e8;background-color:#ffffff;color:#333;"><?= htmlspecialchars($fechaLimiteFormateada) ?></td>
                </tr>
                <?php endif; ?>
              </table>

              <div style="background-color:#f0f8fb;border:1px solid #1993b8;border-radius:6px;padding:16px;margin-bottom:20px;">
                <p style="margin:0 0 8px;color:#22404b;"><strong>Descripción del compromiso</strong></p>
                <p style="margin:0;color:#333;white-space:pre-line;"><?= nl2br(htmlspecialchars($descripcion)) ?></p>
              </div>

              <p style="margin:0 0 20px;color:#333;line-height:1.5;">
                Puedes ver y actualizar el avance de este compromiso desde la sección <strong>Mis compromisos</strong> del sistema.
              </p>
