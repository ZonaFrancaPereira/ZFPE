<?php
/** Contenido del manual de Administrador/Operaciones. Incluido desde administrador.php. */
?>
...
<div class="card border-0 shadow-sm mb-4" style="background:linear-gradient(135deg,var(--zf-navy) 0%,var(--zf-teal) 100%);">
  <div class="card-body py-4 text-white">
    <div class="row align-items-center">
      <div class="col-md-8">
        <h4 class="fw-bold mb-1"><i class="bi bi-shield-fill-check me-2"></i>Manual del Administrador</h4>
        <p class="mb-2 opacity-75">Zona Franca Internacional de Pereira · Centro de Control Gerencial</p>
        <div class="d-flex flex-wrap gap-2">
          <span class="badge bg-white text-zf-navy"><i class="bi bi-shield-fill-check me-1"></i>Administrador</span>
          <span class="badge bg-white text-zf-teal"><i class="bi bi-person-fill-gear me-1"></i>Operaciones</span>
        </div>
      </div>
      <div class="col-md-4 text-md-end mt-3 mt-md-0">
        <i class="bi bi-shield-fill-check opacity-25" style="font-size:5rem;"></i>
      </div>
    </div>
  </div>
</div>
...
<div class="card shadow-sm mb-4">
  <div class="card-body py-2">
    <div class="d-flex flex-wrap gap-2">
      ...
      <a href="#usuarios-sistema" class="btn btn-outline-secondary btn-sm"><i class="bi bi-people-fill me-1"></i>Usuarios</a>
      <a href="#empresas"       class="btn btn-outline-secondary btn-sm"><i class="bi bi-building me-1"></i>Empresas</a>
      <a href="#smtp"           class="btn btn-outline-secondary btn-sm"><i class="bi bi-envelope-gear me-1"></i>Correo (SMTP)</a>
    ...
    <div class="row g-0">
      ...
      <div class="col-md-4 border-end p-4">
        ...
        <ul class="list-unstyled small">
          ...
          <li class="mb-1"><i class="bi bi-check-circle-fill text-success me-2"></i>Configurar la cuenta de correo (SMTP)</li>
      ...
      <div class="col-md-4 border-end p-4">
        ...
        <ul class="list-unstyled small">
          ...
          <li class="mb-1"><i class="bi bi-x-circle-fill text-danger me-2"></i>No accede a Usuarios del sistema ni a SMTP</li>
        </ul>
        <p class="small text-muted mb-0">Tiene su propio manual — ver <strong>Manual de Operaciones</strong> desde el selector de manuales.</p>
      </div>
      ...
      <div class="col-md-4 p-4">
        ...
        <p class="small text-muted mb-0">Tiene su propio manual — ver <strong>Manual de Empresa</strong> y <strong>Manual de Usuario</strong> desde el selector de manuales.</p>
      </div>
...
<!-- ══════════════════════════════════════════════════════ -->
<!-- SECCIÓN: USUARIOS DEL SISTEMA (solo admin)            -->
<!-- ══════════════════════════════════════════════════════ -->
<div id="usuarios-sistema" class="card shadow-sm mb-4">
  <div class="card-header bg-zf-navy text-white">
    <h5 class="mb-0"><i class="bi bi-people-fill me-2"></i>Usuarios del sistema <small class="fw-normal opacity-75">(solo administrador)</small></h5>
  </div>
  <div class="card-body">
    <p class="mb-3">Desde <code>Usuarios</code> en el menú lateral, el administrador crea y gestiona las cuentas internas del sistema (no las de las empresas clientes, esas se manejan desde la ficha de cada empresa).</p>
    <div class="row g-4">
      <div class="col-md-6">
        <h6 class="fw-bold"><i class="bi bi-person-plus text-success me-2"></i>Crear un usuario</h6>
        <div class="bg-body-secondary rounded p-3 small mb-2">
          <code>Usuarios → + Nuevo usuario</code>
        </div>
        <ul class="small mb-0">
          <li>Nombre completo</li>
          <li>Correo electrónico (es el usuario de acceso)</li>
          <li>Contraseña inicial</li>
          <li><strong>Rol:</strong> Administrador, Operaciones o Usuario</li>
        </ul>
      </div>
      <div class="col-md-6">
        <h6 class="fw-bold"><i class="bi bi-pencil-square text-primary me-2"></i>Editar / eliminar</h6>
        <p class="small mb-0">Desde la lista de usuarios se puede editar el nombre, correo y rol, o eliminar la cuenta. No se puede cambiar la contraseña de otro usuario desde aquí — cada quien la cambia desde su propio <strong>Perfil</strong>.</p>
      </div>
    </div>
  </div>
</div>
...
<!-- ══════════════════════════════════════════════════════ -->
<!-- SECCIÓN: CONFIGURACIÓN SMTP (solo admin)              -->
<!-- ══════════════════════════════════════════════════════ -->
<div id="smtp" class="card shadow-sm mb-4">
  <div class="card-header bg-zf-teal text-white">
    <h5 class="mb-0"><i class="bi bi-envelope-gear me-2"></i>Configuración de correo (SMTP) <small class="fw-normal opacity-75">(solo administrador)</small></h5>
  </div>
  <div class="card-body">
    <p class="mb-3">El sistema envía correos de notificación (bienvenida de empresas y usuarios, comités programados, compromisos asignados, requisitos completados) usando una cuenta de correo configurada aquí — hoy conectada a Hostinger.</p>
    <div class="bg-body-secondary rounded p-3 small mb-3">
      <code>Configuración SMTP → Host, puerto, usuario, contraseña, cifrado, correo remitente</code>
    </div>
    <ul class="small mb-3">
      <li>La contraseña se guarda <strong>cifrada</strong> en la base de datos, nunca en texto plano.</li>
      <li>Si se deja el campo de contraseña en blanco al editar, se conserva la contraseña ya guardada.</li>
      <li>Puerto típico: <strong>587 (TLS)</strong> o <strong>465 (SSL)</strong>, según lo indique el proveedor de correo.</li>
    </ul>
    <div class="alert alert-info small mb-0">
      <i class="bi bi-send-check me-1"></i>
      Usa el botón <strong>"Enviar correo de prueba"</strong> después de guardar para confirmar que la conexión funciona antes de depender de ella.
    </div>
  </div>
</div>
...
<div id="cronograma" class="card shadow-sm mb-4">
  ...
  <div class="card-body">
    ...
    <p class="mb-3">Vista visual del avance del proyecto de una empresa, por fases, etapas y requisitos. Operaciones/admin puede ver el cronograma de <strong>cualquier</strong> empresa desde <code>Cronograma → Ver todas las empresas</code>.</p>
  ...
  <div class="card-body">
    <div class="row g-2">
      ...
      $flujo = [
        ...
        ['n'=>'3','color'=>'zf-navy', 'texto'=>'Configurar la cuenta de correo SMTP y enviar un correo de prueba','icon'=>'bi-envelope-gear'],
        ['n'=>'4','color'=>'success', 'texto'=>'Crear empresa y asignarle las etapas que le aplican (el dropdown muestra por fase)','icon'=>'bi-building'],
        ['n'=>'5','color'=>'zf-teal', 'texto'=>'Crear usuario(s) de la empresa para que puedan ingresar','icon'=>'bi-person-plus'],
        ['n'=>'6','color'=>'info',    'texto'=>'Hacer seguimiento periódico a los requisitos (agrupados por fase)','icon'=>'bi-clipboard-check'],
        ['n'=>'7','color'=>'warning', 'texto'=>'Subir documentos soporte requeridos','icon'=>'bi-upload'],
        ['n'=>'8','color'=>'secondary','texto'=>'Registrar comités y compromisos si los hay','icon'=>'bi-people'],
        ['n'=>'9','color'=>'success', 'texto'=>'Revisar reportes y cronograma (separados por fase) para detectar vencidos','icon'=>'bi-bar-chart-fill'],
      ];
      foreach ($flujo as $f):
      ...
      <div class="col-12">
        <div class="d-flex align-items-center gap-3 p-2 rounded border">
          <div class="rounded-circle <?= $esZf ? 'bg-' . $f['color'] : 'bg-' . $f['color'] ?> text-white d-flex align-items-center justify-content-center fw-bold flex-shrink-0"
               style="width:32px;height:32px;font-size:.85rem;">
            <?= $f['n'] ?>
          </div>
          <i class="bi <?= $f['icon'] ?> <?= $esZf ? 'text-' . $f['color'] : 'text-' . $f['color'] ?> flex-shrink-0 fs-5"></i>
